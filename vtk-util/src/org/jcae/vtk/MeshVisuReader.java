...
public class MeshVisuReader extends MeshReader
{
		...
		try
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				if(i == 0)
					...
					System.out.println("READING FREE EDGES");
				...
				// Read the edges
				TIntArrayList leafEdges = new TIntArrayList(nbrOfEdgesComponents / 2);
	...
	public MeshVisu buildPreparationMesh(int leave)
	{
		...
		return constructEdges(mesh);
	}

	private MeshVisu constructEdges(Mesh mesh)
	{
		...
		for (Triangle tri : mesh.getTriangles())
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				{
					freeEdges.add(edge.origin().getLabel());
					freeEdges.add(edge.destination().getLabel());
				} else
				{
					edges.add(edge.origin().getLabel());
					edges.add(edge.destination().getLabel());
				}
			}
		}
		
		toReturn.edges = edges.toNativeArray();
		toReturn.freeEdges = edges.toNativeArray();
