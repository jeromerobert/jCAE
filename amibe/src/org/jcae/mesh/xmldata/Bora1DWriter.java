...
public class Bora1DWriter
{
	private static Logger logger=Logger.getLogger(Bora1DWriter.class);
	
	/**
	 * Used by {@link writeObject}
	 */
	private static Element writeObjectNodes(Document document, Iterator nodesIterator, File nodesFile, File reffile, String baseDir, Mesh mesh)
		throws IOException
	{
		//save nodes
...
/*		// Append elements to <nodes>
	 ...
	 */
	public static void writeObject(Mesh mesh, String xmlDir, String xmlFilename, String brepDir, String brepFile)
	{
		//  Compute node labels
		//mesh.updateNodeLabels();
		try
		{
			...
			for (Iterator it = mesh.subshapeIterator(1); it.hasNext(); )
			{
				CADEdge E = (CADEdge) it.next();
				Mesh m = mesh.cadToMesh(E);
				SubMesh1D submesh = (SubMesh1D) m.mesh;
				...
				// Create <nodes>
				subMeshElement.appendChild(writeObjectNodes(document, submesh.getNodesIterator(), nodesFile, reffile, xmlDir, mesh));
				// Create <beams>
